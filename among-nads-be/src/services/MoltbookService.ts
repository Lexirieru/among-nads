...
export class MoltbookService extends EventEmitter {
  ...
  public async fetchComments(limit = 500): Promise<void> {
    ...
    console.log(`🦞 MoltbookService: Seeded ${this.comments.length} fallback comments`);
    ...
    try {
      ...
      if (allComments.length > 0) {
        ...
        console.log(`🦞 MoltbookService: Replaced with ${this.comments.length} comments (API + fallbacks)`);
      }
    } catch (error) {
